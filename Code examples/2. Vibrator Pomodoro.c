...
void SetIdleMode(void) {
	...
	TCCR0B = 2<<CS00;           // Counter prescaler 1:8, that's 125000 cps
...
int main(void) {
	...
	while(1) {
		...
		if (idlecounter >= 1500) { // Make this a lot shorter when debugging
			SetActiveMode();
			activecounter1 = 0;
			activecounter2 = 0;
			...
			while(activecounter1 < 150) { // Make this a lot shorter when debugging
				...
				if (activecounter2 <= 1250) {
					OCR0B = 50*activecounter2/1250;
					} else if (activecounter2 < 2500) {
					OCR0B = 50*(2500-activecounter2)/1250;
					} else {
					activecounter1++;
					activecounter2 = 0;
				}
