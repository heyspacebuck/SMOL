// TEENY TINY PWMODORO
// Idle for 25 minutes; gently pulse a motor on PB1 for five minutes; repeat
...
void SetActiveMode(void) {
  ...
  ICR0 = 49;                         // Gives a PWM freq. of 125000/50 = 2500 Hz, good for driving a motor of this size (I think)
...
int main(void) {
  ...
  signed int idlecounter = 9999;
  signed int activecounter1 = 149;
  ...
  while(1) {
    ...
    if (idlecounter >= 1250) { // Make this a lot shorter when debugging
      SetActiveMode();
      activecounter2 = 0;
      
      // Sample Active Mode program: pulse up and down
      while(activecounter1 < 150) { // Make this a lot shorter when debugging
        
        OCR0A_idle();
        activecounter2++;
        
        if (activecounter2 <= 2500) {
          OCR0B = 50*activecounter2/2500;
          } else if (activecounter2 < 5000) {
          OCR0B = 50*(5000-activecounter2)/2500;
          } else {
          activecounter1++;
          activecounter2 = 0;
        }
